...
interface SRelChartProps {
  riskMetrics: RiskMetrics
  hoveredDate: Date | null
  onHoverDate: (date: Date | null) => void
}

export function SRelChart({ riskMetrics, hoveredDate, onHoverDate }: SRelChartProps) {
  ...
  const chartData = useMemo(() => {
    if (!riskMetrics?.dates || !riskMetrics?.sRel) {
      return {
        labels: [],
        datasets: [],
      }
    ...
    const datasets = [
      {
        label: METRIC_NAMES.sRel.short,
        data: formatTimeSeriesData(riskMetrics.sRel),
        borderColor: colors.sRel,
        backgroundColor: `${colors.sRel}1A`,
        borderWidth: 2,
        fill: false,
        pointRadius: 0,
        tension: 0.1,
  ...
  const options = useMemo(() => {
    const opts = createTimeSeriesChartOptions(METRIC_NAMES.sRel.long, METRIC_NAMES.sRel.short, "linear")
      ...
      opts.plugins.tooltip.callbacks = {
        label: (context: any) => {
          if (context.parsed.y === null || context.parsed.y === undefined) {
            return `${METRIC_NAMES.sRel.short}: N/A`
          }
          return `${METRIC_NAMES.sRel.short}: ${context.parsed.y.toFixed(3)}`
        },
      }
    }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{METRIC_NAMES.sRel.long}</CardTitle>
            <Button
              variant="ghost"
              size="icon"
