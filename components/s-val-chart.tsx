...
export function SValChart({ riskMetrics, hoveredDate, onHoverDate, cutoffDate = "", onCutoffDateChange, onRecalculate }: SValChartProps) {
  ...
  const chartData = useMemo(() => {
    if (!riskMetrics?.dates || !riskMetrics?.sVal) {
      return {
        labels: [],
        datasets: [],
      }
    ...
    const datasets = [
      {
        label: METRIC_NAMES.sVal.short,
        data: formatTimeSeriesData(riskMetrics.sVal),
        borderColor: colors.sVal,
        backgroundColor: `${colors.sVal}1A`,
        borderWidth: 2,
        fill: false,
        pointRadius: 0,
        tension: 0.1,
  ...
  const options = useMemo(() => {
    const opts = createTimeSeriesChartOptions(METRIC_NAMES.sVal.long, METRIC_NAMES.sVal.short, "linear")
      ...
      opts.plugins.tooltip.callbacks = {
        label: (context: any) => {
          if (context.parsed.y === null || context.parsed.y === undefined) {
            return `${METRIC_NAMES.sVal.short}: N/A`
          }
          return `${METRIC_NAMES.sVal.short}: ${context.parsed.y.toFixed(3)}`
        },
      }
    }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{METRIC_NAMES.sVal.long}</CardTitle>
            <Button
              variant="ghost"
              size="icon"
          ...
          <div className="mt-4 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Use only data up to this date for {METRIC_NAMES.sVal.short} percentile calculation. Leave empty to use all data.
            </p>
          </div>
        </CardHeader>
        <CardContent>
