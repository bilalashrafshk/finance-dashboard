...
export function CPISection() {
  ...
  const loadCPIData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch CPI data')
      }

      const result: CPIResponse = await response.json()
      ...
      setError(err.message || 'Failed to load CPI data')
      toast({
        title: "Error",
        description: err.message || 'Failed to load CPI data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Pakistan CPI National (Year-on-Year)',
        font: {
          size: 16,
          ...
          {latestValue !== null && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Current CPI (YoY)</div>
                <div className={`text-2xl font-bold mt-1 flex items-center gap-2 ${
                  latestValue >= 0 ? 'text-foreground' : 'text-foreground'
                }`}>
