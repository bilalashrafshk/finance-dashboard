...
const SERIES_KEY = 'TS_GP_BOP_WR_M.WR0340'

export function RemittancesSection() {
  ...
  const loadRemittancesData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch remittances data')
      }

      const result: RemittancesResponse = await response.json()
      ...
      toast({
        title: "Error",
        description: err.message || 'Failed to load remittances data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Workers' Remittances to Pakistan",
        font: {
          size: 16,
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(context: any) {
            return `${context.dataset.label}: ${context.parsed.y.toFixed(2)} Million USD`
          },
        },
      },
    },
          ...
          {latestValue !== null && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Current Remittances</div>
