...
const SERIES_KEY = 'TS_GP_BAM_M2_W.M000070'

export function M2Section() {
  ...
  const loadM2Data = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch M2 data')
      }

      const result: M2Response = await response.json()
      ...
      toast({
        title: "Error",
        description: err.message || 'Failed to load M2 data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
          ...
          {latestValue !== null && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Current M2</div>
