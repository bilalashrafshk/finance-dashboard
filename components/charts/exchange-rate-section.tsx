...
export function ExchangeRateSection() {
  ...
  const [allData, setAllData] = useState<ExchangeRateData[]>([]) // Store all fetched data
  ...
  const loadExchangeRateData = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: err.message || 'Failed to load exchange rate data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Average Exchange Rate: PKR per USD',
        font: {
          size: 16,
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(context: any) {
            return `${context.dataset.label}: PKR ${context.parsed.y.toFixed(2)}`
          },
        },
      },
    },
      ...
      y: {
        ...
        ticks: {
          ...
          callback: function(value: any) {
            return typeof value === 'number' ? `PKR ${value.toFixed(0)}` : value
          },
        },
        grid: {
          color: colors.grid,
