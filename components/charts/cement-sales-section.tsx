...
interface CementSalesResponse {
  seriesKey: string
  seriesName: string
  data: CementSalesData[]
  count: number
...
const SERIES_KEY = 'TS_GP_RLS_CEMSEC_M.C_001000'

export function CementSalesSection() {
  const { theme } = useTheme()
  const colors = getThemeColors()
  const { toast } = useToast()
  const [allData, setAllData] = useState<CementSalesData[]>([]) // Store all fetched data
  ...
  const loadData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch cement sales data')
      }
      const result: CementSalesResponse = await response.json()
      ...
      toast({
        title: "Error",
        description: err.message || 'Failed to load cement sales data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const chartOptions = useMemo(() => ({
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(context: any) {
            return `${context.dataset.label}: ${context.parsed.y.toFixed(1)} Thousand Metric Ton`
          },
        },
      },
    },
    scales: {
      ...
      y: {
        display: true,
        title: { display: true, text: 'Thousand Metric Ton', color: colors.foreground },
        ticks: {
          color: colors.foreground,
          callback: function(value: any) {
            return typeof value === 'number' ? `${value.toFixed(0)}K` : value
          },
        },
        grid: { color: colors.grid },
      },
              ...
              {change !== null && (
                <div className="p-4 border rounded-lg">
                  ...
                  <div className={`text-2xl font-bold mt-1 flex items-center gap-2 ${
                    ...
                    {change > 0 ? '+' : ''}{change.toFixed(1)}K MT
                  </div>
                  {changePercent !== null && (
                    <div className="text-xs text-muted-foreground mt-1">
