...
interface VehicleSalesResponse {
  seriesKey: string
  seriesName: string
...
const SERIES_KEY = 'TS_GP_RLS_PSAUTO_M.TAS_001000'

export function VehicleSalesSection() {
  const { theme } = useTheme()
  const colors = getThemeColors()
  const { toast } = useToast()
  const [allData, setAllData] = useState<VehicleSalesData[]>([]) // Store all fetched data
  ...
  const loadData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch vehicle sales data')
      }
      const result: VehicleSalesResponse = await response.json()
      ...
      toast({
        title: "Error",
        description: err.message || 'Failed to load vehicle sales data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Total Sales of Vehicles (except 2 & 3 Wheelers)',
    ...
    scales: {
      ...
      y: {
        display: true,
        title: { display: true, text: 'Number of Vehicles', color: colors.foreground },
        ticks: {
          color: colors.foreground,
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-2xl font-bold mt-1 flex items-center gap-2">
                  ...
                  {latestValue.toLocaleString()} units
                </div>
              ...
              {change !== null && (
                <div className="p-4 border rounded-lg">
                  ...
                  <div className={`text-2xl font-bold mt-1 flex items-center gap-2 ${
                    ...
                    {change > 0 ? '+' : ''}{change.toLocaleString()} units
                  </div>
                  {changePercent !== null && (
                    <div className="text-xs text-muted-foreground mt-1">
