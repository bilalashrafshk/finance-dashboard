...
const SERIES_KEY = 'TS_GP_BAM_SIRKIBOR_D.KIBOR0030'

export function KIBORSection() {
  const { theme } = useTheme()
  const colors = getThemeColors()
  const { toast } = useToast()
  const [data, setData] = useState<KIBORData[]>([])
  ...
  const loadKIBORData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch KIBOR data')
      }

      const result: KIBORResponse = await response.json()
      ...
      setError(err.message || 'Failed to load KIBOR data')
      toast({
        title: "Error",
        description: err.message || 'Failed to load KIBOR data',
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: '6-Months Karachi Interbank Offer Rate (KIBOR)',
        font: {
          size: 16,
          ...
          {latestValue !== null && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Current KIBOR</div>
